$(function(){
    var my_id = $('#staff_id').text();
    view_staff_info(my_id)
})
// 员工总数
var staff_count = 0;
// 当前位置
...
function view_staff_info(my_id) {
    ...
    var info_htmls="";
    var pre = "<div class='swiper-container'><div class='container swiper-wrapper' id='staff_info' style='padding-left:0;position: relative;left: -2%;'>";      
    CallApi(api_url, {}, function (response) {
        ...
        if (rows.length > 0) {
            ...
            rows.forEach(function(row, index, array) {
                    ...
                    <div class="weui-cell">\
                        <div class="upload">\
                            ...
                            <div class="page__hd" >\
                                ...
                                <h3 class="page__desc">'+ row.staff_position + ' ' +'<img  style = "width:14px;height:14px;" src="' + staff_status + '" class = "status">' + ' ' + row.staff_cd + '</h3>\
                            </div>\
                        </div>\
                    </div>\
                ...
                ';
                info_htmls += info_html;
            });
        }
        var end = "</div><div class='swiper-pagination'></div></div><script src='js/pre_image.js'></script>";  

        var all = pre + info_htmls + end;
        $("#staff_info").append(all);
